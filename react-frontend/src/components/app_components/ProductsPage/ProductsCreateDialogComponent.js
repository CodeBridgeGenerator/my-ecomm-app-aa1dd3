...
const ProductsCreateDialogComponent = (props) => {
    ...
    const onSave = async () => {
        if(!validate()) return;
        let _data = {
            name: _entity?.name,description: _entity?.description,price: _entity?.price,stock: _entity?.stock,image: _entity?.image,
...
<div className="col-12 md:col-6 field">
            <span className="align-items-center">
                ...
                <InputNumber id="stock" className="w-full mb-3 p-inputtext-sm" value={_entity?.stock} onChange={(e) => setValByKey("stock", e.value)}  />
            </span>
            <small className="p-error">
            {!_.isEmpty(error["stock"]) ? (
              <p className="m-0" key="error-stock">
                {error["stock"]}
              </p>
            ) : null}
          </small>
