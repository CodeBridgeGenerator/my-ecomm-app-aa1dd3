...
const CartCreateDialogComponent = (props) => {
    ...
    const onSave = async () => {
        if(!validate()) return;
        let _data = {
            cartItems: _entity?.cartItems,totalPrice: _entity?.totalPrice,quantity: _entity?.quantity,
    ...
    return (
        <Dialog header="Create Cart" visible={props.show} closable={false} onHide={props.onHide} modal style={{ width: "40vw" }} className="min-w-max scalein animation-ease-in-out animation-duration-1000" footer={renderFooter()} resizable={false}>
            ...
            <span className="align-items-center">
                <label htmlFor="cartItems">CartItems:</label>
                <InputText id="cartItems" className="w-full mb-3 p-inputtext-sm" value={_entity?.cartItems} onChange={(e) => setValByKey("cartItems", e.target.value)}  />
            </span>
            <small className="p-error">
            {!_.isEmpty(error["cartItems"]) ? (
              <p className="m-0" key="error-cartItems">
                {error["cartItems"]}
              </p>
            ) : null}
          </small>
